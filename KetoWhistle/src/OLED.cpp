...
void OLED_test_loop(Adafruit_SSD1306 *display) {
  ...
  display->clearDisplay();
  ready_sig(display, 3000);

  load(display, 6);

  display->clearDisplay();
  Devicetext(display, "Invaild ! ", 10, 10, 2, false);
  display->display();
  ...
  for(int h = 0; h < 7; h+=1){
    ...
    String vString =  String(count, 2);
    display->clearDisplay();
    Devicetext(display, "Acetone:        ", 20, 3, 1, false);
    Devicetext(display, vString, 30, 15, 2, false);
    Devicetext(display, "ppm", 95, 20, 1, false);
    display->display();
    count +=0.11;
    delay(2000);  
  }
  ...
  for(int m=0; m<10; m+=1){
    ...
    float count=1.87;
    String vString =  String(count, 2);
    Devicetext(display, "BAD! ", 25, 1, 2, false);
    Devicetext(display, vString, 25, 25, 1, false);
    Devicetext(display, "ppm", 55, 25, 1, false);
